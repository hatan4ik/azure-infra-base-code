...
if ! az group show -n "$rgName" >/dev/null 2>&1; then
  ...
  tag_merge "/subscriptions/${subId}/resourceGroups/${rgName}"
fi
...
else sa="$saDesired"; fi
echo "   - SA: $sa"

# ADLS Gen2 SA
if ! az storage account show -g "$rgName" -n "$sa" >/dev/null 2>&1; then
  az storage account create -g "$rgName" -n "$sa" -l "$location"         --sku Standard_LRS --kind StorageV2 --min-tls-version TLS1_2         --allow-blob-public-access false --https-only true --hns true         --tags "${TAGS[@]}" -o none
else
  saId="$(az storage account show -g "$rgName" -n "$sa" --query id -o tsv)"
  tag_merge "$saId"
  hns="$(az storage account show -g "$rgName" -n "$sa" --query isHnsEnabled -o tsv)"
  if [[ "$hns" != "true" ]]; then echo " ! HNS=false; ADLS features need new account"; fi
fi
saId="$(az storage account show -g "$rgName" -n "$sa" --query id -o tsv)"
...
if [[ -n "${LAW_NAME:-}" ]]; then
  ...
  if [[ -n "$LAW_ID" ]]; then
    ...
    if az monitor diagnostic-settings list --resource "$saId" -o tsv --query "value[?name=='ds-sa'].name" | grep -q .; then
      ...
      az monitor diagnostic-settings create --name ds-sa --resource "$saId" --workspace "$LAW_ID" --logs "$logs" --metrics "$metrics" >/dev/null 2>&1 ||           az monitor diagnostic-settings create --name ds-sa --resource "$saId" --workspace "$LAW_ID" --logs "$logs" --metrics "$metrics" >/dev/null
    fi
  fi
fi

# Harden blob service (HNS-aware)
if [[ "$hnsEnabled" == "true" ]]; then
  ...
  az storage account blob-service-properties update -g "$rgName" -n "$sa"         --enable-delete-retention true --delete-retention-days 30         --enable-versioning true --enable-container-delete-retention true         --container-delete-retention-days 7 -o none
fi

# MI RBAC
MI_PID="$(az identity show -g "${MI_RESOURCE_GROUP}" -n "${MI_NAME}" --query principalId -o tsv 2>/dev/null || true)"
if [[ -n "$MI_PID" ]]; then
  if ! az role assignment list --assignee-object-id "$MI_PID" --scope "$saId" --query "[?roleDefinitionName=='Storage Blob Data Contributor']|[0]" -o tsv | grep -q .; then
    az role assignment create --assignee-object-id "$MI_PID" --role "Storage Blob Data Contributor" --scope "$saId" -o none || echo "   ! RBAC grant failed"
  fi
fi

# Private Endpoints (one per groupId)
if [[ "${ENABLE_STORAGE_PE}" == "true" ]]; then
  NET_RG="${NET_RG}"; VNET_NAME="${VNET_NAME}"; SNET_PE_NAME="${SNET_PE_NAME}"
  ...
  if ! az network private-endpoint show -g "$rgName" -n "$PE_BLOB" >/dev/null 2>&1; then
    az network private-endpoint create -g "$rgName" -n "$PE_BLOB" -l "$location"           --subnet "$SUBNET_ID"           --private-connection-resource-id "$saId"           --group-ids blob           --connection-name "$CONN_BLOB"           --tags "${TAGS[@]}" -o none
  fi
  if ! az network private-endpoint dns-zone-group show -g "$rgName" --endpoint-name "$PE_BLOB" -n "dzg-${sa}-blob" >/dev/null 2>&1; then
  ...
  # dfs (only for HNS=true)
  if [[ "$hnsEnabled" == "true" ]]; then
    PE_DFS="pe-${sa}-dfs"; CONN_DFS="conn-${sa}-dfs"
    if ! az network private-endpoint show -g "$rgName" -n "$PE_DFS" >/dev/null 2>&1; then
      az network private-endpoint create -g "$rgName" -n "$PE_DFS" -l "$location"             --subnet "$SUBNET_ID"             --private-connection-resource-id "$saId"             --group-ids dfs             --connection-name "$CONN_DFS"             --tags "${TAGS[@]}" -o none
    fi
    if ! az network private-endpoint dns-zone-group show -g "$rgName" --endpoint-name "$PE_DFS" -n "dzg-${sa}-dfs" >/dev/null 2>&1; then
...
echo "✓ Customer '${CUSTOMER_SLUG}' storage ensured."
